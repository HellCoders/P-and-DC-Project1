...
int main(int argc, char** argv) {
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        double end = clock();
        minSerial = std::min(minSerial, (end - start)/CLOCKS_PER_SEC);
    }
    ...
    for (int i = 0; i < 5; ++i) {
        ...
        double end = clock();
        minISPC = std::min(minISPC, (end - start)/CLOCKS_PER_SEC);
    }

    printf("[mandelbrot ispc]:\t\t[%.3f] ms\n", minISPC * 1000);
    //ritePPMImage(output_ispc, width, height, "mandelbrot-ispc.ppm", maxIterations);
    ...
    if (useTasks) {
        ...
        for (int i = 0; i < 3; ++i) {
            ...
            double end = clock();
            minTaskISPC = std::min(minTaskISPC, (end - start)/CLOCKS_PER_SEC);
        }
